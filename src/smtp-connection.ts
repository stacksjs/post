...
class SMTPConnection extends EventEmitter {
  ...
  handler_MAIL(command, callback) {
    const parsed = this._parseAddressCommand('mail from', command)
  ...
  upgrade(callback, secureCallback) {
    ...
    // upgrade connection
    const secureSocket = new tls.TLSSocket(this._socket, socketOptions)
